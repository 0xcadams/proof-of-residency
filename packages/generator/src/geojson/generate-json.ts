...
export const generate = async () => {
  const allCountries: AllCountries = JSON.parse(
    fs
      .readFileSync(`./src/geojson/original/countries-${defaultCountriesResolution}.geo.json`)
      .toString()
  );
  const allCountriesHigherResolution: AllCountries = JSON.parse(
    fs
      .readFileSync(`./src/geojson/original/countries-${highCountriesResolution}.geo.json`)
      .toString()
  );
  ...
  for (let country of allCountries.features.slice(
    ...
    const countryMultiPolygon = turf.toMercator(countryMultiPolygonNonProjected, { mutate: true });
    ...
    const heightOffset = width > height ? (width - height) / 2 / widthOrHeight : 0;
    const widthOffset = height > width ? (height - width) / 2 / widthOrHeight : 0;

    const scaledToWidthHeight = countryMultiPolygon.geometry.coordinates.map((outer) =>
      outer.map((inner) =>
        inner.map((coordinate) => {
          ...
          const x = Math.abs(coordinate[0] - projectedBbox[0]) / widthOrHeight + widthOffset;
          const y = Math.abs(coordinate[1] - projectedBbox[3]) / widthOrHeight + heightOffset;
    ...
    const waterBodiesResolution =
      actualArea < 1e10 ? '250m' : actualArea < 1e11 ? '500m' : actualArea < 1e12 ? '1km' : '2km5';
    ...
    const waterBodies: AllWaterBodies = JSON.parse(
      fs
        .readFileSync(`./src/geojson/original/earth-waterbodies-${waterBodiesResolution}.geo.json`)
        .toString()
    );
    ...
    const bar = new ProgressBar(':bar  :percent :etas', {
      total: mercatorWaterBodies.geometries.length * countryMultiPolygon.geometry.coordinates.length
    });
      ...
      .flatMap((hydro) => {
        ...
        return countryMultiPolygon.geometry.coordinates.map((polygon) => {
          try {
            ...
            return intersectionMultiPolygon.geometry.coordinates.flatMap((outer) =>
              outer.map((inner) =>
                inner.map((coordinate) => {
                  const x =
                    Math.abs((coordinate[0] - projectedBbox[0]) / widthOrHeight) + widthOffset;
                  const y =
                    Math.abs((coordinate[1] - projectedBbox[3]) / widthOrHeight) + heightOffset;
  ...
  console.log(`----- Saved ${countCountries} countries total, out of ${iso.all().length}`);
};
